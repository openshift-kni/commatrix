...
func Run(o *GenerateOptions) (err error) {
	...
	var listeningCheck *listeningsockets.ConnectionCheck
	if o.openPorts {
		if listeningCheck, err = generateSS(o); err != nil {
			return fmt.Errorf("failed to generate SS matrix: %w", err)
		}
	}

	// If format is all in one, merge the SS matrix and the normal matrix and set the listeningCheck back to nil as we
	// do not want to write those files.
	if isAllInOne(o) && listeningCheck != nil {
		matrix = matrix.Merge(listeningCheck.SSCommMatrix)
		listeningCheck = nil
	}
	...
	if listeningCheck != nil {
		...
		diff := matrixdiff.Generate(matrix, listeningCheck.SSCommMatrix)
...
func generateSS(o *GenerateOptions) (*listeningsockets.ConnectionCheck, error) {
	if o.debug {
		log.SetLevel(log.DebugLevel)
	}
	...
	if err := listeningCheck.GenerateSS(consts.DefaultDebugNamespace); err != nil {
		return nil, fmt.Errorf("error while generating the listening check matrix and ss raws: %v", err)
	}

	return listeningCheck, nil
}
...
// isAllInOne returns true for formats that combine both static and open ports
// into a single output (Butane and MachineConfig formats).
func isAllInOne(o *GenerateOptions) bool {
