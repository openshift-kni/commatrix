...
Flags:
      ...
      --custom-node-group stringArray    Assign nodes to a custom group for separate firewall CRs (format: groupName=node1,node2,...). Repeatable.
  ```
...
When a subset of worker nodes run additional services, use `--custom-node-group` to split them into a custom group with separate firewall rules:

```sh
$ oc commatrix generate --format butane \
    --custom-node-group mc-ingress=worker-2,worker-5,worker-6
```

This produces separate output per group:
- `butane-master.yaml` — master firewall rules
- `butane-worker.yaml` — rules for workers not in a special group
- `butane-mc-ingress.yaml` — rules for the ingress workers (includes their extra ports + static entries inherited from the worker role)
