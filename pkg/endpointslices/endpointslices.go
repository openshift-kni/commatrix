...
// PoolRoles returns roles per observed pool.
func (ep *EndpointSlicesExporter) PoolRoles() map[string][]string {
	return ep.poolToRoles
}
...
func New(cs *client.ClientSet) (*EndpointSlicesExporter, error) {
	...
	poolToRoles, err := mcp.MapPoolToRoles(cs, nodeToPool)
	if err != nil {
		return nil, err
	}
	return &EndpointSlicesExporter{cs, nodeToPool, []EndpointSlicesInfo{}, poolToRoles}, nil
}
