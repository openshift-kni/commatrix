...
// filterEndpointPortsExcludeLocalhost returns endpoint ports from the given pod
// but excludes any port entries explicitly bound to the localhost IPv4 address (127.0.0.1).
func filterEndpointPortsExcludeLocalhost(portsInfo []EndpointPortInfo) []discoveryv1.EndpointPort {
	filtered := make([]discoveryv1.EndpointPort, 0, len(portsInfo))
	for _, pi := range portsInfo {
		if pi.ContainerPort.HostIP == "127.0.0.1" {
			continue
		}
