...
var _ = g.Describe("Commatrix creator pkg tests", func() {
	g.Context("Get Costume entries List From File", func() {
		for _, format := range []string{types.FormatCSV, types.FormatJSON, types.FormatYAML} {
			g.It(fmt.Sprintf("Should successfully extract ComDetails from a %s file", format), func() {
				...
				cm, err := New(nil, fmt.Sprintf("../../samples/custom-entries/example-custom-entries.%s", format), format, configv1.BareMetalPlatformType, configv1.HighlyAvailableTopologyMode, false, false, nil)
		...
		g.It("Should return an error due to non-matched customEntriesPath and customEntriesFormat types", func() {
			...
			cm, err := New(nil, "../../samples/custom-entries/example-custom-entries.csv", types.FormatJSON, configv1.BareMetalPlatformType, configv1.HighlyAvailableTopologyMode, false, false, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should return an error due to an invalid customEntriesFormat", func() {
			...
			cm, err := New(nil, "../../samples/custom-entries/example-custom-entries.csv", types.FormatNFT, configv1.BareMetalPlatformType, configv1.HighlyAvailableTopologyMode, false, false, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
	...
	g.Context("Get static entries from file", func() {
		g.It("Should successfully get static entries suitable to baremetal standard cluster", func() {
			...
			cm, err := New(nil, "", "", configv1.BareMetalPlatformType, configv1.HighlyAvailableTopologyMode, false, false, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should successfully get static entries suitable to baremetal SNO cluster", func() {
			...
			cm, err := New(nil, "", "", configv1.BareMetalPlatformType, configv1.SingleReplicaTopologyMode, false, false, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should successfully get static entries suitable to None standard cluster", func() {
			...
			cm, err := New(nil, "", "", configv1.NonePlatformType, configv1.HighlyAvailableTopologyMode, false, false, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should successfully get static entries suitable to None SNO cluster", func() {
			...
			cm, err := New(nil, "", "", configv1.NonePlatformType, configv1.SingleReplicaTopologyMode, false, false, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should include DHCP static entries when dhcpEnabled is true on standard cluster", func() {
			...
			cm, err := New(nil, "", "", configv1.BareMetalPlatformType, configv1.HighlyAvailableTopologyMode, false, true, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should include DHCP static entries when dhcpEnabled is true on SNO cluster", func() {
			...
			cm, err := New(nil, "", "", configv1.BareMetalPlatformType, configv1.SingleReplicaTopologyMode, false, true, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should return an error due to an invalid value for cluster environment", func() {
			...
			cm, err := New(nil, "", "", "invalid", configv1.SingleReplicaTopologyMode, false, false, nil)
			o.Expect(err).ToNot(o.HaveOccurred())
	...
	g.Context("Create EndpointSlice Matrix", func() {
		...
		g.It("Should successfully create an endpoint matrix with custom entries", func() {
			...
			commatrixCreator, err := New(endpointSlices, "../../samples/custom-entries/example-custom-entries.csv", types.FormatCSV, configv1.AWSPlatformType, configv1.SingleReplicaTopologyMode, false, false, mockUtils)
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should successfully create an endpoint matrix without custom entries", func() {
			...
			commatrixCreator, err := New(endpointSlices, "", "", configv1.AWSPlatformType, configv1.SingleReplicaTopologyMode, false, false, mockUtils)
			o.Expect(err).ToNot(o.HaveOccurred())
			commatrix, err := commatrixCreator.CreateEndpointMatrix()
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should include IPv6 static entries when ipv6Enabled is true on Standard", func() {
			...
			commatrixCreator, err := New(endpointSlices, "", "", configv1.AWSPlatformType, configv1.HighlyAvailableTopologyMode, true, false, mockUtils)
			o.Expect(err).ToNot(o.HaveOccurred())
			commatrix, err := commatrixCreator.CreateEndpointMatrix()
			o.Expect(err).ToNot(o.HaveOccurred())
		...
		g.It("Should filter out localhost-bound ports from endpoint matrix", func() {
			...
			commatrixCreator, err := New(localhostEndpointSlices, "", "", configv1.AWSPlatformType, configv1.SingleReplicaTopologyMode, false, false, mockUtils)
			o.Expect(err).ToNot(o.HaveOccurred())
			commatrix, err := commatrixCreator.CreateEndpointMatrix()
			o.Expect(err).ToNot(o.HaveOccurred())
